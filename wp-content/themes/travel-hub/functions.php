...
function travel_hub_enqueue_styles() {
    ...
    wp_enqueue_style('travel-hub-child-style', get_stylesheet_directory_uri() .'/style.css', array('adventure-travelling-style'));

    require get_theme_file_path( '/tp-theme-color.php' );
    wp_add_inline_style( 'travel-hub-child-style',$adventure_travelling_tp_theme_css );
    
    wp_style_add_data('travel-hub-child-style', 'rtl', 'replace');
...
function travel_hub_header_style() {
    ...
    $custom_header = "
        ...
        wp_add_inline_style( 'travel-hub-child-style', $custom_header );
    endif;
}
...
function travel_hub_meta_callback_trip_offer( $post ) {
    ...
    $trip_amount = get_post_meta( $post->ID, 'travel_hub_trip_amount', true );
    $trip_days = get_post_meta( $post->ID, 'travel_hub_trip_days', true );
    ?>
        ...
        <table id="list">
            <tbody id="the-list" data-wp-lists="list:meta">
                <tr id="meta-8">
                    ...
                    <td class="left">
                        <input type="text" name="travel_hub_trip_amount" id="travel_hub_trip_amount" value="<?php echo esc_attr($trip_amount); ?>" />
                    </td>
                </tr>
                <tr id="meta-8">
                    <td class="left">
                    ...
                    <td class="left">
                        <input type="text" name="travel_hub_trip_days" id="travel_hub_trip_days" value="<?php echo esc_attr($trip_days); ?>" />
                    </td>
                </tr>
            </tbody>
        </table>
...
if ( ! function_exists( 'travel_hub_credit' ) ) {
    function travel_hub_credit(){
        echo "<a href=".esc_url(TRAVEL_HUB_CREDIT)." target='_blank'>".esc_html__('Travel WordPress Theme','travel-hub')."</a>";
    }
}
